/* eslint-disable */
// Vendored from https://github.com/grafana/k6-jslib-summary/blob/main/src/text.js
// Avoids a dependency on the remote jslib.k6.io CDN which may be unreachable in CI.
...
function humanizeDuration(dur, timeUnit) {
  if (timeUnit !== '' && Object.prototype.hasOwnProperty.call(unitMap, timeUnit)) {
    return (dur * unitMap[timeUnit].coef).toFixed(2) + unitMap[timeUnit].unit;
  }
  return humanizeGenericDuration(dur);
}

export function humanizeValue(val, metric, timeUnit) {
  if (metric.type == 'rate') {
