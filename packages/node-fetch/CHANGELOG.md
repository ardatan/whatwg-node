...
- [#3504](https://github.com/ardatan/whatwg-node/pull/3504)
  ...
  When `tls.getCACertificates` is available (Node.js 22.15+ / 23.10+), the
  libcurl fetch implementation now loads CAs from
  ...
  On older Node versions (engines still allow `>=18`), behavior is unchanged:
  `NODE_EXTRA_CA_CERTS` still maps to libcurl `CAINFO`, otherwise the bundled
