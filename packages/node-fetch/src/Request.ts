...
import { PonyfillHeaders, PonyfillHeadersInit, isHeadersLike } from './Headers.js';
...
export class PonyfillRequest<TJSON = any> extends PonyfillBody<TJSON> implements Request {
  constructor(input: RequestInfo | URL, options?: RequestPonyfillInit) {
    ...
    this.headers = (requestInit?.headers && isHeadersLike(requestInit.headers)) ? requestInit.headers : new PonyfillHeaders(requestInit?.headers);
