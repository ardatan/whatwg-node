    ...
    if (!init) {
      init = {
        headers: {
          ...
          'content-length': bodyInit.length.toString(),
        },
      };
      ...
      if (!init.headers.has('content-length')) {
        init.headers.set('content-length', bodyInit.length.toString());
      }
      ...
      if (!contentLengthExists) {
        init.headers.push(['content-length', bodyInit.length.toString()]);
      }
      ...
      if (init.headers?.['content-length'] == null) {
        init.headers['content-length'] = bodyInit.length.toString();
      }
    }
