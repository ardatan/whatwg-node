...
describe('Request Abort', () => {
  runTestsForEachServerImpl((server, serverImplName) => {
    // Abort + undici sockets are flaky under Jest --detectLeaks; unit tests still
    // cover the undici abort path. Leak runs keep node-http here.
    runTestsForEachFetchImpl(
      (implementationName, { fetchAPI, createServerAdapter }) => {
        skipIf(
        ...
        // assert the socket itself. Skip undici/uWS/Bun/Deno (different write paths).
        skipIf(
          implementationName === 'undici' ||
