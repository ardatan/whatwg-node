...
The floor is set to **22.15** (not just 22.0) so we can rely on `tls.getCACertificates()` and always-on `zlib` zstd helpers (`createZstdCompress` / `createZstdDecompress`), which landed in Node.js 22.15 / 23.10. That matches the oldest currently supported LTS line (22 Maintenance) while dropping only EOL majors.
